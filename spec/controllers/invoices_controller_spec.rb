...
describe InvoicesController do
  ...
  describe "GET index" do
    it "assigns all invoices as @invoices" do
      ...
      get :index
      assigns(:invoices).should eq([mock_invoice])
    end
  end
  ...
  describe "GET show" do
    it "assigns the requested invoice as @invoice" do
      ...
      get :show, :id => "37"
      assigns(:invoice).should be(mock_invoice)
    end
  end

  describe "GET new" do
    it "assigns a new invoice as @invoice" do
      Invoice.stub(:new) { mock_invoice }
      get :new
      assigns(:invoice).should be(mock_invoice)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created invoice as @invoice" do
        Invoice.stub(:new).with({'these' => 'params'}) { mock_invoice(:save => true) }
        post :create, :invoice => {'these' => 'params'}
        assigns(:invoice).should be(mock_invoice)
      end

      it "redirects to the created invoice" do
        Invoice.stub(:new) { mock_invoice(:save => true) }
        post :create, :invoice => {}
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved invoice as @invoice" do
        Invoice.stub(:new).with({'these' => 'params'}) { mock_invoice(:save => false) }
        post :create, :invoice => {'these' => 'params'}
        assigns(:invoice).should be(mock_invoice)
      end

      it "re-renders the 'new' template" do
        Invoice.stub(:new) { mock_invoice(:save => false) }
        post :create, :invoice => {}
  ...
  describe "PUT update" do
    ...
    describe "with valid params" do
      it "updates the requested invoice" do
        Invoice.should_receive(:find).with("37") { mock_invoice }
